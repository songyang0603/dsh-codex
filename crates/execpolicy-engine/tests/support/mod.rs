...
impl Sidecar {
    pub fn spawn() -> Self {
        let mut child = Command::new(env!("CARGO_BIN_EXE_dsh-codex-execpolicy-engine"))
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .expect("spawn compiled exec-policy sidecar");
    ...
    #[allow(dead_code)] // Used by protocol tests; this module is compiled per integration test crate.
...
#[allow(dead_code)] // Used by protocol tests; this module is compiled per integration test crate.
pub fn assert_error<'a>(response: &'a Value, code: &str) -> &'a Value {
    assert!(
