...
function pointerWidth(arch) {
  const value = { arm64: 64, x64: 64 }[arch];
  if (value === undefined)
    throw new Error(`unsupported Node architecture: ${arch}`);
  return value;
}
...
function usage() {
  return [
    "Usage: node scripts/stage-approval-native.mjs [options]",
    "",
    "Options:",
...
function expectedHello() {
  ...
  return {
    ...
    codexAppServerProtocolPackage: "codex-app-server-protocol",
    codexAppServerProtocolVersion: "0.0.0",
    codexAppServerProtocolTree: components.appServerProtocol.gitObject,
    codexAppServerProtocolV2Tree: components.appServerProtocolV2.gitObject,
    commandApprovalItemBlob: components.approvalAppServerV2Item.gitObject,
    commandApprovalPermissionsBlob:
      components.approvalAppServerV2Permissions.gitObject,
    codexCargoLockBlob: components.cargoLock.gitObject,
    methods: [
      "parse_command_execution_request_approval",
...
async function verifyHello(binary, platform, arch) {
  ...
  const response = new Promise((resolveResponse, rejectResponse) => {
    const timeout = setTimeout(() => {
      child.kill();
      rejectResponse(new Error("approval native sidecar hello timed out"));
    }, 10_000);
    ...
    child.once("exit", (code, signal) => {
      ...
      rejectResponse(
        new Error(
          `approval native sidecar exited before hello: code=${code} signal=${signal}; ${stderr}`,
        ),
      );
    });
  });
  ...
  child.stdin.write(
    `${JSON.stringify({ protocolVersion: 1, id: 1, method: "hello", params: {} })}\n`,
  );
  const message = await response;
  if (message.id !== 1 || message.error !== undefined || !message.result) {
    child.kill();
    throw new Error("approval native sidecar returned a malformed hello");
  }
  for (const [field, expected] of Object.entries(expectedHello())) {
    if (JSON.stringify(message.result[field]) !== JSON.stringify(expected)) {
      child.kill();
      throw new Error(
        `approval native provenance mismatch at ${field}: expected ${JSON.stringify(expected)}, got ${JSON.stringify(message.result[field])}`,
      );
    }
  }
  for (const [field, expected] of [
    ["os", rustOs(platform)],
    ["arch", rustArch(arch)],
    ["pointerWidth", pointerWidth(arch)],
  ]) {
    if (message.result[field] !== expected) {
      child.kill();
      throw new Error(
        `approval native target mismatch at ${field}: expected ${expected}, got ${String(message.result[field])}`,
      );
    }
  }
  child.stdin.write(
  ...
  try {
    const outcome = await Promise.race([
      ...
      new Promise((_, rejectExit) => {
        shutdownTimeout = setTimeout(() => {
          child.kill();
          rejectExit(
            new Error("approval native sidecar did not exit after shutdown"),
          );
        }, 5_000);
      }),
    ]);
    ...
    if (outcome.code !== 0) {
      throw new Error(
        `approval native sidecar shutdown failed: code=${outcome.code} signal=${outcome.signal}`,
      );
    }
  } finally {
...
async function main() {
  ...
  process.stdout.write(
    `staged ${options.platform}-${options.arch} approval protocol engine\n${destination}\nsha256 ${sha256}\n`,
  );
}
